...
const AppHeader = ({ onPartnerClick, onBookClick, onScrollTo }) => {
  ...
  return (
    <header className="h-14 flex items-center justify-between px-6 border-b border-slate-800/50 bg-slate-950 sticky top-0 z-50">
      {/* Logo */}
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="sm:hidden w-8 h-8 flex items-center justify-center rounded-lg border border-slate-700 text-slate-300 bg-slate-900"
      >
        {open ? "✕" : "☰"}
      </button>
      ...
      {open && (
        <>
          {/* Backdrop */}
          <div
            className="sm:hidden fixed inset-0 top-14 bg-black/60 z-40"
            onClick={() => setOpen(false)}
          />
          {/* Menu */}
          <div className="sm:hidden absolute left-0 right-0 top-14 bg-slate-950 border-b border-slate-800 z-50 shadow-2xl">
            <div className="flex flex-col px-6 py-4 gap-0.5">
              {navLinks.map((item) => (
                <button
                  key={item.label}
                  onClick={() => handleAndClose(() => onScrollTo(item.id))}
                  className="text-left py-3 text-sm text-slate-300 hover:text-yellow-300 border-b border-slate-800/60 last:border-0 transition-colors"
                >
                  {item.label}
                </button>
              ))}
              <div className="flex gap-2 mt-3">
                <button
                  ...
                  className="flex-1 py-3 rounded-full bg-yellow-400 text-slate-950 text-sm font-bold"
                >
                  Book Now
                </button>
                <button
